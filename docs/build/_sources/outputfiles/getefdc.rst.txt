...
`GetEFDC` is a Fortran utility to read the binary output files produced by EFDC+. `GetEFDC` is a a starting point for the user to create their own analysis of EFDC+ output.  It is expected that a user utilizing this tool has knowledge of Fortran and can modify `GetEFDC` to meet their specific needs. 
