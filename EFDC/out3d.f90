SUBROUTINE OUT3D
  ...
  INTEGER :: IAD,JAD,KP,K,L,LN,LS,J,I,NW
  REAL    :: ASALMAX,ASALMIN,TMPVAL,ATEMMAX,ATEMMIN,ADYEMAX,ADYEMIN,ASEDMAX,ASEDMIN,AUUUMAX
  REAL    :: AUUUMIN,AVVVMAX,AVVVMIN,AWWWMAX,AWWWMIN,ASNDMAX,ASNDMIN,ATOXMAX,ATOXMIN,ZZPS,SCALE3D
  CHARACTER *11 SALFN,TEMFN,DYEFN,SEDFN,UUUFN,VVVFN,WWWFN,CMPFN,SNDFN,TOXFN
  ...
  IF( NCALL3D == 1 )THEN
    OPEN(50,FILE=OUTDIR//'OUT3D.DIA',STATUS='UNKNOWN')
    ...
    ENDDO
  ELSE
    OPEN(50,FILE=OUTDIR//'OUT3D.DIA',POSITION='APPEND' &
           ,STATUS='UNKNOWN')
    WRITE(50,530)NCALL3D
  ENDIF
  IF( IS3DSAL >= 1 )THEN
    ...
    DO K=1,KC
      DO L=2,LA
        ...
        TMPVAL=0.5*(W(L,K)+W(L,K-1)) &
            +GI*ZZ(L,K)*( DTI*(P(L)-P1(L)) &
            +0.5*(U(LEC(L),K)*(P(LEC(L))-P(L))*DXIU(LEC(L)) &
            +U(L,K)*(P(L)-P(LWC(L)))*DXIU(L) &
            +V(LN,K)*(P(LN)-P(L))*DYIV(LNC(L)) &
            +V(L,K)*(P(L)-P(LS))*DYIV(L)) ) &
            +0.5*(1.-ZZ(L,K))*( U(LEC(L),K)*(BELV(LEC(L))-BELV(L))*DXIU(LEC(L)) &
            +U(L,K)*(BELV(L)-BELV(LWC(L)))*DXIU(L) &
