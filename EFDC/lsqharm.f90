SUBROUTINE LSQHARM
  ...
  ! CHANGE RECORD
  ! **  SUBROUTINE LSQHARM PERFORMS A LEAST SQUARES HARMONIC ANALYSIS

  USE GLOBAL
  IMPLICIT NONE
  INTEGER :: MG
  INTEGER :: MC,ML,MR,I,J,K,M,L,LN,MCP1,MCPP,MRP1,MRPP
  ...
  REAL :: TIME,TMOD,UTMP1,VTMP1,UTMP,VTMP,AMP,PHI,PHASE,UMEAN,VMEAN
  REAL :: TERM1,TERM2,TERM3,TERM4,RPLUS,RMINS,APLUS,AMINS,DET
  ...
  IF( JSLSHA == 0 ) GOTO 100
  JSLSHA=0
  !
  ! **  INITIALIZE COEFFICIENT ARRAYS AND CONSTANTS
  !
  RMLSHA=0.
  RTLSHA=0.
  RTTLSHA=0.
...
100 CONTINUE
  ...
  DO ML=1,MLLSHA
    ...
    UTMP1=0.5*(UHDYE(LEC(L))+UHDYE(L))/(DYP(L)*HP(L))
  ...
  !
    200 CONTINUE
  OPEN(97,FILE=OUTDIR//'LSHA.OUT',STATUS='UNKNOWN')
  CLOSE(97,STATUS='DELETE')
  OPEN(97,FILE=OUTDIR//'LSHA.OUT',STATUS='UNKNOWN')
  IF( ISLSTR == 1 ) GOTO 500
  !
  ! *** COMPUTE SOLUTION WITH NO TREND REMOVAL
  ! *** LOAD GLOBAL COEFFICIENT MATRIX
  !
  MG=2*MTIDE+1
  GLSHA(1,1)=RMLSHA
  DO MC=1,MTIDE
  ...
  WRITE(97,13)
  !
  ! *** SOLVE BY BACK SUBSTITUTION AND OUTPUT RESULTS
  !
  ...
  DO ML=1,MLLSHA
    IF( LSHAP(ML) == 1 )THEN
      ...
      DO MC=1,MTIDE
        MCP1=MC+1
        MCPP=MC+1+MTIDE
        AMP=SQRT(RSOL(MCP1)*RSOL(MCP1) + RSOL(MCPP)*RSOL(MCPP))
        IF( RSOL(MCPP) == 0.0 .AND. RSOL(MCP1) == 0.0 )THEN
          PHI=999999.
        ELSE
          PHI=ATAN2(RSOL(MCPP),RSOL(MCP1))
        ENDIF
        PHASE=TCP(MC)*PHI/PI2
        IF( PHASE < 0.) PHASE=PHASE+TCP(MC)
        AMATMP(MC)=AMP
  ...
  DO ML=1,MLLSHA
    IF( LSHAUE(ML) == 1 )THEN
      ...
      DO MC=1,MTIDE
        MCP1=MC+1
        MCPP=MC+1+MTIDE
        AMPU(MC)=SQRT(RSOL(MCP1)*RSOL(MCP1) + RSOL(MCPP)*RSOL(MCPP))
        UCOS(MC)=RSOL(MCP1)
        USIN(MC)=RSOL(MCPP)
        IF( RSOL(MCPP) == 0.0 .AND. RSOL(MCP1) == 0.0 )THEN
          PHI=999999.
        ...
        IF( PHASEU(MC) < 0.) PHASEU(MC)=PHASEU(MC)+TCP(MC)
      ENDDO
      RHS(1)=VELSHA(ML)
      DO MC=1,MTIDE
        MCP1=MC+1
      ...
      DO MC=1,MTIDE
        MCP1=MC+1
        MCPP=MC+1+MTIDE
        AMPV(MC)=SQRT(RSOL(MCP1)*RSOL(MCP1) + RSOL(MCPP)*RSOL(MCPP))
        VCOS(MC)=RSOL(MCP1)
        VSIN(MC)=RSOL(MCPP)
        IF( RSOL(MCPP) == 0.0 .AND. RSOL(MCP1) == 0.0 )THEN
          PHI=999999.
        ...
        IF( PHASEV(MC) < 0.) PHASEV(MC)=PHASEV(MC)+TCP(MC)
      ENDDO
      DO MC=1,MTIDE
        ...
        IF( PHASEE(MC) < 0.) PHASEE(MC)=PHASEE(MC)+TCP(MC)
      ENDDO
      WRITE(97,10)TITLE
      WRITE(97,14)CLSL(ML),ILLSHA(ML),JLLSHA(ML)
  ...
  GOTO 501
    500 CONTINUE
  !
  ! *** COMPUTE SOLUTION WITH TREND REMOVAL
  ! *** COMPUTE TREND
  !
  ...
  DO ML=1,MLLSHA
    IF( LSHAP(ML) == 1 )THEN
      ...
      DO MC=1,MTIDE
        MCPP=MTIDE+MC
        AMP=SQRT(RSOL(MC)*RSOL(MC) + RSOL(MCPP)*RSOL(MCPP))
        IF( RSOL(MCPP) == 0.0 .AND. RSOL(MC) == 0.0 )THEN
          PHI=999999.
        ELSE
  ...
  DO ML=1,MLLSHA
    IF( LSHAU(ML) == 1 )THEN
      ...
      DO MC=1,MTIDE
        MCPP=MC+MTIDE
        AMPU(MC)=SQRT(RSOL(MC)*RSOL(MC) + RSOL(MCPP)*RSOL(MCPP))
        UCOS(MC)=RSOL(MC)
        USIN(MC)=RSOL(MCPP)
        IF( RSOL(MCPP) == 0.0 .AND. RSOL(MC) == 0.0 )THEN
          PHI=999999.
      ...
      DO MC=1,MTIDE
        MCPP=MC+MTIDE
        AMPV(MC)=SQRT(RSOL(MC)*RSOL(MC) + RSOL(MCPP)*RSOL(MCPP))
        VCOS(MC)=RSOL(MC)
        VSIN(MC)=RSOL(MCPP)
        IF( RSOL(MCPP) == 0.0 .AND. RSOL(MC) == 0.0 )THEN
          PHI=999999.
  ...
  ENDDO
     ...
     24 FORMAT('  SYMBOL',3X,'TCP',12X,'AMU',12X,'PHU',12X,'AMV',12X,'PHV',/)
     25 FORMAT(1X,A5,3X,E12.4,3X,E12.4,3X,E12.4,3X,E12.4,3X,E12.4,/)
     26 FORMAT('  SYMBOL',3X,'TCP',12X,'MAJ',12X,'MIN',12X,'ANG',12X,'PHE',/)
     40 FORMAT(1X,F16.3)
     41 FORMAT(//)
     42 FORMAT(F15.0)
